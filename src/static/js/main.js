...
function drawRect(box, classNo) {
    [y1, x1, y2, x2] = box;
    if (x1 === x2 || y1 === y2) return;
    camera_ctx.lineWidth = 3;
    camera_ctx.strokeStyle = colors[classNo];
    camera_ctx.strokeRect(
        x1*camera_canvas.width,
        y1*camera_canvas.height,
        (x2-x1)*camera_canvas.width,
        (y2-y1)*camera_canvas.height
    );
    camera_ctx.fillStyle = colors[classNo];
    camera_ctx.fillRect(
        x1*camera_canvas.width-1,
        y1*camera_canvas.height-12,
        camera_ctx.measureText(labels[classNo-1]).width+10,
        13
    );
